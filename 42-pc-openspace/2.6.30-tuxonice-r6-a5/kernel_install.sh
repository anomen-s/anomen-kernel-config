...
echo "kernel" "/boot/kernel-$KERVER" "root=$ROOT" "ro" "acpi_pm_good" >> /boot/grub/grub.conf
echo "title=Linux $KERVER noresume" >> /boot/grub/grub.conf
echo "kernel" "/boot/kernel-$KERVER" "root=$ROOT" "ro" "acpi_pm_good" "noresume" "nox" >> /boot/grub/grub.conf
